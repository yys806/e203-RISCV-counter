# 基于 E203 的四位数码管计数器 - 课程设计报告

## 1. 摘要
Summary based on README: peripheral, hierarchy, registers, scan, firmware, sim, board.

## 2. Keywords
RISC-V, E203, ICB, 7-seg, FPGA, BCD, scan, firmware, ram.hex, Gowin

## 3. Background and Requirements
ICB peripheral on E203, supports 1Hz auto-count and CPU write control, keep 18MHz clock and led_my polarity.

## 4. System Overview
Hierarchy seg_out/cc passthrough; base 0x10014000; CTRL bit0=count_en; DATA BCD; 18MHz -> 200Hz -> 1Hz.

## 5. RTL Design
my_periph_example ICB read/write, divider, BCD inc, active-high seg/cc; e203_subsys_* adds ports; cst binds led_my pins.

## 6. Firmware
_init then CTRL=0/DATA=0000; loop BCD write + printf; delay_ms via CLINT.

## 7. Build and Load
make clean && make (firmware/hello_world/Debug) -> ram.hex; ITCM readmemh; Gowin synth/P&R/download.

## 8. Simulation
sim/iverilog-lnx script; extend sys_tb_top.sv finish time if needed; watch ICB writes and scan outputs.

## 9. Board Test
seg_out/cc per cst; UART0 115200; expect 0000 then 1Hz updates and UART prints.

## 10. Issues
usleep link -> delay_ms; script path; ram.hex path via run dir or symlink; sim time adjust.

## 11. Future
add dp/modes/irq/power, auto tests.

## 12. Conclusion
ICB peripheral + firmware achieve software-controlled display; sim/synth/board path clear.
